...
public class EvaluationItemController {
    ...
    @GetMapping("/api/admin/evaluation-items")
    public ApiResponse<EvaluationItemSearchResponse> listEvaluationItems(
        ...
        requireEvaluationItemAdmin(servletRequest);
        return ApiResponse.ok(service.list(new EvaluationItemSearchCriteria(page, size, ruleVersionId, areaCode, activeYn, keyword)), effectiveRequestId(requestId));
    }

    @PostMapping("/api/admin/evaluation-items/save")
    public ApiResponse<EvaluationItemRow> saveEvaluationItem(
            @Valid @RequestBody SaveEvaluationItemRequest request,
            @RequestHeader(value = "X-Request-Id", required = false) String requestId,
            HttpServletRequest servletRequest) {
        CurrentUser user = requireEvaluationItemAdmin(servletRequest);
    ...
    private CurrentUser requireEvaluationItemAdmin(HttpServletRequest request) {
