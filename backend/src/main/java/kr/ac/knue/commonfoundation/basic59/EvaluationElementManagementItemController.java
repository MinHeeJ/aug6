...
@RestController
public class EvaluationElementManagementItemController {
    private static final Set<String> ALLOWED_ROLES = Set.of("R04", "R09");
    private final EvaluationElementManagementItemService service;

    public EvaluationElementManagementItemController(EvaluationElementManagementItemService service) {
        this.service = service;
    }

    @GetMapping("/api/business/evaluation-element-management-items")
    public ApiResponse<EvaluationElementManagementItemSearchResponse> listEvaluationElementManagementItems(
        ...
        return ApiResponse.ok(service.list(new EvaluationElementManagementItemSearchCriteria(page, pageSize, evaluationYear, areaCode, elementCode)), effectiveRequestId(requestId));
    }

    @PostMapping("/api/business/evaluation-element-management-items/save")
    public ApiResponse<EvaluationElementManagementItemRow> saveEvaluationElementManagementItem(
            @Valid @RequestBody SaveEvaluationElementManagementItemRequest request,
            @RequestHeader(value = "X-Request-Id", required = false) String requestId,
            HttpServletRequest servletRequest) {
    ...
    private void validatePageSize(int pageSize) {
        if (pageSize != 20 && pageSize != 50 && pageSize != 100) {
            throw new BusinessValidationException("평가요소별 관리항목 목록 표시 건수가 올바르지 않습니다.",
