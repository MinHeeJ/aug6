...
@RestController
public class BusinessPeriodController {
    private static final Set<String> ADMIN_ROLES = Set.of("R03", "R04", "R09");
    private final BusinessPeriodService service;

    public BusinessPeriodController(BusinessPeriodService service) {
        this.service = service;
    }

    @GetMapping("/api/admin/business-periods")
    public ApiResponse<BusinessPeriodSearchResponse> listBusinessPeriods(
    ...
    @PostMapping("/api/admin/business-periods/save")
    public ApiResponse<BusinessPeriodSettingRow> saveBusinessPeriod(
            @Valid @RequestBody SaveBusinessPeriodRequest request,
            @RequestHeader(value = "X-Request-Id", required = false) String requestId,
            HttpServletRequest servletRequest) {
    ...
    private void validatePageSize(int size) {
        if (size != 20 && size != 50 && size != 100) {
            throw new BusinessValidationException("평가·업적입력 기간 목록 표시 건수가 올바르지 않습니다.",
