...
@RestController
public class EvaluationDateController {
    private static final Set<String> ADMIN_ROLES = Set.of("R03", "R04", "R09");
    private final EvaluationDateService service;

    public EvaluationDateController(EvaluationDateService service) {
        this.service = service;
    }

    @GetMapping("/api/admin/evaluation-dates")
    public ApiResponse<EvaluationDateSearchResponse> listEvaluationDates(
    ...
    @PostMapping("/api/admin/evaluation-dates/save")
    public ApiResponse<BusinessPeriodSettingRow> saveEvaluationDate(
            @Valid @RequestBody SaveEvaluationDateRequest request,
            @RequestHeader(value = "X-Request-Id", required = false) String requestId,
            HttpServletRequest servletRequest) {
    ...
    private void validatePageSize(int size) {
        if (size != 20 && size != 50 && size != 100) {
            throw new BusinessValidationException("평가일자 목록 표시 건수가 올바르지 않습니다.",
