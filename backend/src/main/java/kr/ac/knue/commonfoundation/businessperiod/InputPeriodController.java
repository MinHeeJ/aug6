...
@RestController
public class InputPeriodController {
    private static final Set<String> ADMIN_ROLES = Set.of("R03", "R04", "R09");
    private final InputPeriodService service;

    public InputPeriodController(InputPeriodService service) {
        this.service = service;
    }

    @GetMapping("/api/admin/input-periods")
    public ApiResponse<InputPeriodSearchResponse> listInputPeriods(
    ...
    @PostMapping("/api/admin/input-periods/save")
    public ApiResponse<BusinessPeriodSettingRow> saveInputPeriod(
            @Valid @RequestBody SaveInputPeriodRequest request,
            @RequestHeader(value = "X-Request-Id", required = false) String requestId,
            HttpServletRequest servletRequest) {
    ...
    private void validatePageSize(int size) {
        if (size != 20 && size != 50 && size != 100) {
            throw new BusinessValidationException("입력기간 목록 표시 건수가 올바르지 않습니다.",
