...
@RestController
public class ModificationPeriodController {
    private static final Set<String> ADMIN_ROLES = Set.of("R03", "R04", "R09");
    private final ModificationPeriodService service;

    public ModificationPeriodController(ModificationPeriodService service) {
        this.service = service;
    }

    @GetMapping("/api/admin/modification-periods")
    public ApiResponse<ModificationPeriodSearchResponse> listModificationPeriods(
    ...
    @PostMapping("/api/admin/modification-periods/save")
    public ApiResponse<BusinessPeriodSettingRow> saveModificationPeriod(
            @Valid @RequestBody SaveModificationPeriodRequest request,
            @RequestHeader(value = "X-Request-Id", required = false) String requestId,
            HttpServletRequest servletRequest) {
    ...
    private void validatePageSize(int size) {
        if (size != 20 && size != 50 && size != 100) {
            throw new BusinessValidationException("수정기간 목록 표시 건수가 올바르지 않습니다.",
