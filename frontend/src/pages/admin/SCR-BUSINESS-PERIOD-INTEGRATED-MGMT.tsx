...
export function BusinessPeriodManagementPage() {
  ...
  const [rows, setRows] = useState<BusinessPeriodSetting[]>([]);
  const [selected, setSelected] = useState<BusinessPeriodSetting | null>(null);
  ...
  const selectedLabel = useMemo(() => {
    if (!selected) return "신규 평가·업적입력 기간 등록";
  ...
  const load = async () => {
    try {
      ...
      const response = await businessPeriodApi.listBusinessPeriods({
        ...filters,
        page,
        size: pageSize,
      });
      setRows(response.data?.businessPeriods ?? []);
  ...
  const selectRow = (row: BusinessPeriodSetting) => {
    setSelected(row);
    setFieldErrors({});
  ...
  const save = async () => {
    ...
    const confirmed = window.confirm(
      `${form.evaluationYear} 평가·업적입력 기간 설정을 저장하시겠습니까?`,
    );
    if (!confirmed) return;
    try {
      setSaving(true);
      setError(null);
      setFieldErrors({});
      await businessPeriodApi.saveBusinessPeriod({
  ...
  if (permissionDenied) {
    return (
      <section
        ...
        data-testid="business-period-page"
      >
        <PermissionState
          title="평가·업적입력 기간 관리 권한이 없습니다"
          message="R03, R04, R09 역할 또는 평가·업적입력 기간 관리 메뉴 접근 권한이 필요합니다."
        />
      </section>
    );
  }
  ...
  return (
    ...
    >
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-muted">
              평가연도·소속/학과·평가영역 조합별 평가·업적입력 가능 시작일시와
              종료일시를 통합 조회하고 저장합니다.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <button
              ...
              data-testid="business-period-excel-button"
              onClick={() =>
                setSuccessMessage(
                  "현재 조회 조건의 평가·업적입력 기간 목록을 엑셀로 내려받을 수 있습니다.",
                )
              }
              type="button"
            >
            ...
            <button
              ...
              data-testid="business-period-refresh-button"
              onClick={() => void load()}
              type="button"
            >
      ...
      <div className="sr-only">
        평가·업적입력 기간 관리 권한이 없습니다 조회된 평가·업적입력 기간이
        없습니다 저장되었습니다 loading empty error permission success
        저장하시겠습니까
      </div>
      {error ? (
        <ErrorState title="평가·업적입력 기간 처리 오류" message={error} />
      ) : null}
      {successMessage ? <SuccessState title={successMessage} /> : null}

      <section
        className="rounded-md border border-ld bg-white p-5 shadow-sm"
        data-testid="business-period-search-panel"
      >
        <div className="grid gap-4 md:grid-cols-3 xl:grid-cols-6">
          <TextInput
            label="평가연도"
            testId="business-period-filter-year-input"
          ...
          <TextInput
            label="평가영역 코드"
            testId="business-period-filter-area-input"
            value={filters.areaCode}
            onChange={(value) => setFilters({ ...filters, areaCode: value })}
          />
          <TextInput
            label="소속/학과 코드"
            testId="business-period-filter-organization-input"
          ...
          <TextInput
            label="사용자유형 코드"
            testId="business-period-filter-user-type-input"
          ...
          <label className="text-sm font-medium text-dark">
            ...
            <select
              ...
              data-testid="business-period-filter-active-select"
          ...
          <label className="text-sm font-medium text-dark">
            ...
            <select
              className="mt-1 h-10 w-full rounded-md border border-ld px-3 py-2 text-sm"
              data-testid="business-period-page-size-select"
              value={pageSize}
              onChange={(event) => {
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-[1fr_auto]">
          <TextInput
            label="검색어"
            testId="business-period-keyword-input"
          ...
          <button
            ...
            data-testid="business-period-search-button"
            onClick={() => {
              setPage(0);
              void load();
            }}
      ...
      <div className="grid grid-cols-12 gap-6">
        <section
          ...
          data-testid="business-period-list-panel"
        >
          ...
          {loading ? <LoadingState title="평가·업적입력 기간 조회 중" /> : null}
          {!loading && rows.length === 0 ? (
            <EmptyState
              title="조회된 평가·업적입력 기간이 없습니다"
                ...
                <tbody className="divide-y divide-ld">
                  {rows.map((row) => (
                    <tr
                      ...
                      data-testid={`business-period-row-${row.settingId}`}
                      key={row.settingId}
          ...
          <div className="mt-4 flex justify-end gap-2">
            <button
              ...
              data-testid="business-period-prev-page-button"
              disabled={page === 0}
            ...
            <button
              className="rounded-md border border-ld px-3 py-2 text-sm disabled:opacity-40"
              data-testid="business-period-next-page-button"
        ...
        <aside
          ...
          data-testid="business-period-editor-panel"
        >
          ...
          <div className="mt-4 space-y-4">
            <Field label="평가연도" required error={fieldErrors.evaluationYear}>
              <input
                ...
                data-testid="business-period-year-input"
            ...
            >
              <input
                className="w-full rounded-md border border-ld px-3 py-2 text-sm"
                data-testid="business-period-organization-input"
            ...
            <Field label="평가영역 코드" error={fieldErrors.areaCode}>
              <input
                className="w-full rounded-md border border-ld px-3 py-2 text-sm"
                data-testid="business-period-area-input"
                value={form.areaCode}
                onChange={(event) =>
            ...
            <Field label="사용자유형 코드" error={fieldErrors.userTypeCode}>
              <input
                className="w-full rounded-md border border-ld px-3 py-2 text-sm"
                data-testid="business-period-user-type-input"
            ...
            <Field label="시작일시" required error={fieldErrors.startAt}>
              <input
                className="w-full rounded-md border border-ld px-3 py-2 text-sm"
                data-testid="business-period-start-at-input"
                type="datetime-local"
                value={form.startAt}
                onChange={(event) =>
            ...
            <Field label="종료일시" required error={fieldErrors.endAt}>
              <input
                className="w-full rounded-md border border-ld px-3 py-2 text-sm"
                data-testid="business-period-end-at-input"
                type="datetime-local"
                value={form.endAt}
                onChange={(event) =>
            ...
            <Field label="기준일자" required error={fieldErrors.baseDate}>
              <input
                className="w-full rounded-md border border-ld px-3 py-2 text-sm"
                data-testid="business-period-base-date-input"
                type="date"
                value={form.baseDate}
                onChange={(event) =>
            ...
            <Field label="사용여부" required error={fieldErrors.activeYn}>
              <select
                className="w-full rounded-md border border-ld px-3 py-2 text-sm"
                data-testid="business-period-active-select"
                value={form.activeYn}
                onChange={(event) =>
            ...
            <Field label="변경 사유" required error={fieldErrors.changeReason}>
              <textarea
                ...
                data-testid="business-period-change-reason-textarea"
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            <button
              ...
              data-testid="business-period-save-button"
              disabled={saving}
              onClick={() => void save()}
              type="button"
            >
            ...
            <button
              ...
              data-testid="business-period-cancel-button"
              onClick={resetForm}
              type="button"
            >
              취소
