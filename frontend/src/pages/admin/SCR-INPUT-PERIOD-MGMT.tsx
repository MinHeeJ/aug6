...
export function InputPeriodManagementPage() {
  ...
  const [rows, setRows] = useState<InputPeriodSetting[]>([]);
  const [selected, setSelected] = useState<InputPeriodSetting | null>(null);
  ...
  const load = async () => {
    try {
      ...
      const response = await inputPeriodApi.listInputPeriods({
        ...filters,
        page,
        size: pageSize,
      });
      setRows(response.data?.inputPeriods ?? []);
  ...
  const selectRow = (row: InputPeriodSetting) => {
    setSelected(row);
    setFieldErrors({});
  ...
  const save = async () => {
    ...
    const confirmed = window.confirm(
      `${form.evaluationYear} 입력기간 설정을 저장하시겠습니까?`,
    );
    if (!confirmed) return;
    try {
      setSaving(true);
      setError(null);
      setFieldErrors({});
      await inputPeriodApi.saveInputPeriod({
  ...
  if (permissionDenied) {
    return (
      <section
        data-screen-id="SCR-INPUT-PERIOD-MGMT"
        data-testid="input-period-page"
      >
        <PermissionState
          title="입력기간 관리 권한이 없습니다"
          message="R03, R04, R09 역할 또는 입력기간 관리 메뉴 접근 권한이 필요합니다."
        />
      </section>
    );
  }
    ...
    >
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-sm text-link">
              평가 기준 관리 / 업무기간 관리 / 입력기간 관리
            </p>
            <h1 className="mt-2 text-xl font-semibold text-dark">
              입력기간 관리
            </h1>
            <p className="mt-2 text-sm text-muted">
              평가연도·소속/학과·평가영역 조합별 신규 실적 등록 가능 시작일시와
              종료일시를 조회하고 저장합니다.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <button
              ...
              data-testid="input-period-excel-button"
              onClick={() =>
                setSuccessMessage(
                  "현재 조회 조건의 입력기간 목록을 엑셀로 내려받을 수 있습니다.",
                )
              }
              type="button"
            >
            ...
            <button
              ...
              data-testid="input-period-refresh-button"
              onClick={() => void load()}
              type="button"
            >
      ...
      >
        <div className="grid gap-4 md:grid-cols-3 xl:grid-cols-6">
          ...
          <TextInput
            label="평가영역 코드"
            testId="input-period-filter-area-input"
            value={filters.areaCode}
            onChange={(value) => setFilters({ ...filters, areaCode: value })}
          />
          <TextInput
            label="소속/학과 코드"
            testId="input-period-filter-organization-input"
          ...
          <TextInput
            label="사용자유형 코드"
            testId="input-period-filter-user-type-input"
          ...
          <label className="text-sm font-medium text-dark">
            ...
            <select
              ...
              data-testid="input-period-filter-active-select"
          ...
          <label className="text-sm font-medium text-dark">
            ...
            <select
              className="mt-1 h-10 w-full rounded-md border border-ld px-3 py-2 text-sm"
              data-testid="input-period-page-size-select"
              value={pageSize}
              onChange={(event) => {
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-[1fr_auto]">
          <TextInput
            label="검색어"
            testId="input-period-keyword-input"
          ...
          <button
            ...
            data-testid="input-period-search-button"
            onClick={() => {
              setPage(0);
              void load();
            }}
      ...
      <div className="grid grid-cols-12 gap-6">
        <section
          ...
          data-testid="input-period-list-panel"
        >
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold text-dark">입력기간 목록</h2>
            <span className="text-sm text-muted">총 {totalElements}건</span>
          </div>
          {loading ? <LoadingState title="입력기간 조회 중" /> : null}
          {!loading && rows.length === 0 ? (
            <EmptyState
              title="조회된 입력기간이 없습니다"
                ...
                <tbody className="divide-y divide-ld">
                  {rows.map((row) => (
                    <tr
                      ...
                      data-testid={`input-period-row-${row.settingId}`}
                      key={row.settingId}
          ...
          <div className="mt-4 flex justify-end gap-2">
            <button
              ...
              data-testid="input-period-prev-page-button"
              disabled={page === 0}
            ...
            <button
              className="rounded-md border border-ld px-3 py-2 text-sm disabled:opacity-40"
              data-testid="input-period-next-page-button"
        ...
        <aside
          ...
          data-testid="input-period-editor-panel"
        >
          <div className="flex items-center gap-2">
            <CalendarCheck className="text-primary" size={18} />
            <h2 className="text-lg font-semibold text-dark">입력기간 저장</h2>
          </div>
          ...
          <div className="mt-4 space-y-4">
            <Field label="평가연도" required error={fieldErrors.evaluationYear}>
              <input
                ...
                data-testid="input-period-year-input"
            ...
            >
              <input
                className="w-full rounded-md border border-ld px-3 py-2 text-sm"
                data-testid="input-period-organization-input"
            ...
            <Field label="평가영역 코드" error={fieldErrors.areaCode}>
              <input
                className="w-full rounded-md border border-ld px-3 py-2 text-sm"
                data-testid="input-period-area-input"
                value={form.areaCode}
                onChange={(event) =>
            ...
            <Field label="사용자유형 코드" error={fieldErrors.userTypeCode}>
              <input
                className="w-full rounded-md border border-ld px-3 py-2 text-sm"
                data-testid="input-period-user-type-input"
            ...
            <Field label="시작일시" required error={fieldErrors.startAt}>
              <input
                className="w-full rounded-md border border-ld px-3 py-2 text-sm"
                data-testid="input-period-start-at-input"
                type="datetime-local"
                value={form.startAt}
                onChange={(event) =>
            ...
            <Field label="종료일시" required error={fieldErrors.endAt}>
              <input
                className="w-full rounded-md border border-ld px-3 py-2 text-sm"
                data-testid="input-period-end-at-input"
                type="datetime-local"
                value={form.endAt}
                onChange={(event) =>
            ...
            <Field label="기준일자" required error={fieldErrors.baseDate}>
              <input
                className="w-full rounded-md border border-ld px-3 py-2 text-sm"
                data-testid="input-period-base-date-input"
                type="date"
                value={form.baseDate}
                onChange={(event) =>
            ...
            <Field label="사용여부" required error={fieldErrors.activeYn}>
              <select
                className="w-full rounded-md border border-ld px-3 py-2 text-sm"
                data-testid="input-period-active-select"
                value={form.activeYn}
                onChange={(event) =>
            ...
            <Field label="변경 사유" required error={fieldErrors.changeReason}>
              <textarea
                ...
                data-testid="input-period-change-reason-textarea"
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            <button
              ...
              data-testid="input-period-save-button"
              disabled={saving}
              onClick={() => void save()}
              type="button"
            >
            ...
            <button
              ...
              data-testid="input-period-cancel-button"
              onClick={resetForm}
              type="button"
            >
              취소
